...
from _algopy_testing.utils import as_bytes, as_int64, as_string, static_size_of
...
class Box(typing.Generic[_TValue]):
    ...
    def create(self, *, size: algopy.UInt64 | int | None = None) -> bool:
        ...
        content_type_size = static_size_of(self._type)
        ...
        if content_type_size is not None and size_arg is not None:
            if size_arg < content_type_size:
                raise ValueError(f"Box size cannot be less than {content_type_size}")
        ...
        size_int = (size_arg if size_arg is not None else content_type_size) or 0
